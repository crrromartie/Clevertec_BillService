...
public class AddProductCommand implements Command {
    ...
    public Router execute(HttpServletRequest request) {
        ...
        double price = Double.parseDouble(request.getParameter(ParameterName.PRODUCT_PRICE));
        ServiceFactory factory = ServiceFactory.getINSTANCE();
        ProductService productService = factory.getProductService();
        try {
            if (!productService.isProductUnique(name)) {
                logger.log(Level.DEBUG, "Not unique product name");
                request.setAttribute(AttributeName.NAME_PRODUCT_UNIQUE_ERROR, true);
            ...
            logger.log(Level.ERROR, "Error during add new product!", e.getMessage());
